...
public partial class CameraPreviewPage : ContentPage
{
    ...
    private void SetVendor(string vendor)
    {
        ...
        LblDefaultPass.Text = _isHik
            ? "Hikvision: RTSP port 554, HTTP 8000 | pass de trong"
            : "Provision: RTSP port 544/554/8554 (profile1-3) | pass admin/123456";
    ...
    private async void OnConnectClicked(object? sender, EventArgs e)
    {
        ...
        var explicitUrl = TxtRtsp.Text?.Trim();

        if (string.IsNullOrEmpty(explicitUrl))
        {
            if (string.IsNullOrEmpty(ip))
            {
                LblConnStatus.Text = "Vui long nhap IP camera.";
                return;
            }
        ...
        else if (!explicitUrl.StartsWith("rtsp://", StringComparison.OrdinalIgnoreCase))
        {
            LblConnStatus.Text = "URL RTSP phai bat dau bang rtsp://";
            return;
        }
        ...
        try
        {
            var candidates = string.IsNullOrEmpty(explicitUrl)
                ? new List<string>(_isHik ? BuildHikRtspUrls(ip, user, pass) : BuildProvRtspUrls(ip, user, pass))
                : new List<string> { explicitUrl };

            var host = string.IsNullOrEmpty(ip) ? RtspProbeService.HostOf(candidates[0]) : ip;
            LblConnStatus.Text = "Dang kiem tra cong RTSP...";
            var openPorts = await RtspProbeService.ProbePortsAsync(host, candidates.Select(RtspProbeService.PortOf), 600, CancellationToken.None);
            if (openPorts.Count == 0)
            {
                var httpOpen = (await RtspProbeService.ProbePortsAsync(host, new[] { 80, 8000 }, 600, CancellationToken.None)).Count > 0;
            ...
            var rtspCheck = "";
            var describe = "";
            if (!string.IsNullOrEmpty(explicitUrl) && openPorts.Contains(RtspProbeService.PortOf(explicitUrl)))
            {
                var (rtspUser, rtspPass) = RtspProbeService.ParseCreds(explicitUrl);
                rtspCheck = await RtspProbeService.CheckRtspAsync(explicitUrl, 4000);
                describe = await RtspProbeService.DescribeRtspAsync(explicitUrl, rtspUser, rtspPass, 5000);
                LblConnStatus.Text = $"Kiem tra RTSP: {rtspCheck}\nDESCRIBE: {describe}";
            }

            var tried = new List<string>();
            var ok = UseVlc
                ? await TryPlayVlcCandidatesAsync(candidates, openPorts, tried)
                : await TryPlaySystemCandidatesAsync(candidates, openPorts, tried);
            if (ok) return;
            ...
            var extra = (string.IsNullOrEmpty(rtspCheck) ? "" : $"\nKiem tra RTSP: {rtspCheck}")
                      + (string.IsNullOrEmpty(describe) ? "" : $"\nDESCRIBE: {describe}");
            LblConnStatus.Text = string.IsNullOrEmpty(lastErr)
                ? $"Khong phat duoc video. Da thu {tried.Count} duong dan RTSP (cong mo: {string.Join(", ", openPorts)}). Kiem tra IP/user/pass va duong dan.{extra}{vlcErr}\n\nVLC log:\n{vlcLog}\n\nVLC file log:\n{fileLog}"
                : $"Khong phat duoc video. Loi VLC: {lastErr}{extra}{vlcErr}\n\nVLC log:\n{vlcLog}\n\nVLC file log:\n{fileLog}";
        }
        catch (Exception ex)
        {
        ...
        {
            BtnConnect.IsEnabled = true;
            BtnConnect.Text = "XEM LIVE";
        }
    }

    private async Task<bool> TryPlayVlcCandidatesAsync(System.Collections.Generic.List<string> candidates, HashSet<int> openPorts, System.Collections.Generic.List<string> tried)
    {
    ...
    private async Task<bool> TryPlaySystemCandidatesAsync(System.Collections.Generic.List<string> candidates, HashSet<int> openPorts, System.Collections.Generic.List<string> tried)
    {
        foreach (var rtsp in candidates.Where(c => openPorts.Contains(RtspProbeService.PortOf(c))))
        {
            tried.Add(RtspProbeService.Sanitize(rtsp));
    ...
    // ---------- System engine (VideoView / MediaPlayer) ----------
